...
import me.gavin.photo.viewer.databinding.ItemPhotoViewBinding
 ...
 */
class LargeFragment2 : SupportFragment() {
    ...
    class B(context: Context, list: List<Image>) : RecyclerAdapter<Image, ItemPhotoViewBinding>(context, list, R.layout.item_photo_view) {
        override fun onBind(holder: RecyclerHolder<ItemPhotoViewBinding>?, t: Image?, position: Int) {
            Glide.with(mContext)
                    .fromMediaStore()
        ...
        override fun onViewRecycled(holder: RecyclerHolder<ItemPhotoViewBinding>?) {
